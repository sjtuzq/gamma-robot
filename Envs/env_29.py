...
"""
action 29: lifting something with something on it
"""
...
class Engine29(Engine):
    def __init__(self, opt, worker_id=None):
        super(Engine29,self).__init__(opt)
        self.opt = opt 
        self._wid = worker_id
    ...
    def init_obj(self):
        ...
        self.obj_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/bottles/b1/b1.urdf"),globalScaling=self.obj1_scaling)
        self.obj2_scaling = 1.5
        self.obj2_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/bottles/b6/b6.urdf"),globalScaling=self.obj2_scaling)
    ...
    def init_motion(self):
        self.data_q = np.load (os.path.join(self.env_root,"init/29_0_q.npy"))
        self.data_gripper = np.load(os.path.join(self.env_root,"init/29_0_gripper.npy"))#np.load (self.env_root + '/init/gripper.npy')
        self.robot.setJointValue(self.data_q[0],gripper=self.data_gripper[0])

    def init_grasp(self):
        self.robot.gripperControl(0)

        qlist = np.load (os.path.join(self.env_root,"init/29_0_q.npy"))
        glist = np.load(os.path.join(self.env_root,"init/29_0_gripper.npy"))
        num_q = len(qlist[0])
