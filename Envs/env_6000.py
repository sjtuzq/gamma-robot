#!/usr/bin/env python3
"""
   env for action 6000, consist of the following different tasks:
   action 86: pulling sth from left to right
   action 87: pulling sth from right to left
   action 93: pushing sth from left to right
   action 94: pushing sth from right to left
"""
...
class Engine6000(Engine):
    def __init__(self,opt):
        super(Engine6000,self).__init__(opt)

    def init_grasp(self):
        if self.opt.load_embedding == 86 or self.opt.load_embedding == 94:
            self.init_grasp_right()
        elif self.opt.load_embedding == 87 or self.opt.load_embedding == 93:
            self.init_grasp_left()
        else:
            print("action error!")
            exit()

    def init_grasp_left(self):
        try:
