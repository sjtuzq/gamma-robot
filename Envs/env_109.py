...
"""
action 109:putting something on a surface
"""
...
class Engine109(Engine):
    def __init__(self, opt, worker_id=None):
        super(Engine109,self).__init__(opt)
        self.opt = opt 
        self._wid = worker_id
    ...
    def init_obj(self):
        self.obj_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/bottles/b1/b1.urdf"))
        self.obj2_scaling = 1.5
        self.obj2_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/bottles/b6/b6.urdf"),globalScaling=self.obj2_scaling,useFixedBase=True)
        self.p.changeVisualShape (self.obj2_id, -1, rgbaColor=[1.,0.,0.,1])
   
    def reset_obj(self):
        self.p.resetBasePositionAndOrientation(self.obj_id,[0.3637 + 0.06, -0.05, 0.33],[0, 0, 0, 1.0])
        self.p.resetBasePositionAndOrientation(self.obj2_id,[0.3637, 0.05, 0.29],[0, 0, -0.1494381, 0.9887711])
