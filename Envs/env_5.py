...
class Engine5(Engine):
    def __init__(self, opt,worker_id=None):
        super(Engine5,self).__init__(opt)
    ...
    def init_obj(self):
        #self.obj_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/drawers/d1/d1.urdf"))
        self.obj_v = self.p.createVisualShape(shapeType=self.p.GEOM_MESH,fileName=os.path.join(self.env_root,"urdf/obj_libs/drawers/d1/drawer_new.obj"),meshScale=(0.6,0.6,0.6))
        self.obj_c = self.p.createCollisionShape(shapeType=self.p.GEOM_MESH,fileName=os.path.join(self.env_root,"urdf/obj_libs/drawers/d1/drawer_new.obj"),meshScale=(0.6,0.6,0.6))
        self.obj_id = self.p.createMultiBody(1.0,self.obj_c,self.obj_v) 
        
    
        self.obj2_v = self.p.createVisualShape(shapeType=self.p.GEOM_MESH,fileName=os.path.join(self.env_root,"urdf/obj_libs/drawers/d3/model_new.obj"),meshScale=(0.7,0.9,0.63))
        #self.obj2_id = self.p.loadURDF( os.path.join(self.env_root, "urdf/obj_libs/drawers/d3/d3.urdf"),useFixedBase=True)
        self.obj2_c = self.p.createCollisionShape(shapeType=self.p.GEOM_MESH,fileName=os.path.join(self.env_root,"urdf/obj_libs/drawers/d3/model_new.obj"),flags=self.p.GEOM_FORCE_CONCAVE_TRIMESH,meshScale=(0.8,1.1,0.7))
        self.obj2_id = self.p.createMultiBody(0,self.obj2_c,self.obj2_v) 
    ...
    def reset_obj(self):
        ...
        self.obj1_ori =  p.getQuaternionFromEuler ([math.pi/2.0,0,-math.pi/2.0])
        self.p.resetBasePositionAndOrientation(self.obj_id,[self.obj_x ,  self.obj_y, self.obj_z],self.obj1_ori)
        self.obj2_ori =  p.getQuaternionFromEuler ([-math.pi/2.0-0.05, 0 ,math.pi/2.0])
        self.p.resetBasePositionAndOrientation(self.obj2_id,[self.obj_x + 0.2 , self.obj_y, self.obj_z + 0.02 ],self.obj2_ori)
        ...
        self.p.changeDynamics(self.obj2_id, -1, contactStiffness=10000.0, contactDamping=1)
