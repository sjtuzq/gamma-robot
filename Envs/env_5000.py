#!/usr/bin/env python3
"""
   env for action 5000, consist of the following different tasks:
   action 40: move sth away from sth
   action 42: move sth closer to sth
"""
...
class Engine5000(Engine):
    def __init__(self,opt):
        super(Engine5000,self).__init__(opt)
    ...
    def init_grasp(self):
        ...
        up_traj = point2traj([pos, [pos[0], pos[1]+0.05, pos[2]+0.1]])
        ...
        if self.opt.rand_start == 'rand':
            ...
            pos = p.getLinkState (self.robotId, 7)[0]
            up_traj = point2traj ([pos, [pos[0], pos[1]+(random.random()-0.5)*0.2+0.16, pos[2]+0.04]])
            start_id = self.move (up_traj, orn_traj, start_id)
        ...
        if self.opt.rand_box == 'rand':
            ...
            self.box_position = [0.42+(random.random()-0.5)*0.5, 0.18+(random.random()-0.5)*0.5, 0.33]
            ...
            self.box_file = os.path.join(self.env_root,"urdf/objmodels/urdfs/cup.urdf")
            self.box_position = [0.42, 0.18, 0.33]
            self.box_orientation = self.p.getQuaternionFromEuler([-math.pi/2, 0, 0])
            self.box_scaling = 0.15
