...
def analysis_nlp_embedding(cmd='move sth down'):
    ...
    # camera_data = np.zeros((int(inter_n+1),int(inter_n+1),3))
    #
    # up_embedding = np.load('utils/labels/up_embedding.npy')
    #
    # action_curve = []
    # for i in range (100):
    #     r = 0.01 * i
    #     state[0] = r * env.embedding_data[45] + (1 - r) * env.embedding_data[43]
    #     action = agent.select_action (state)
    #     # print (i, r, action)
    #
    #     obj = env.p.getAABB (env.obj_id, -1)
    ...
    #
    #     print (i, r, action[2])
    #     action_curve.append(action[2])
    # plt.plot(action_curve)
    # plt.show()


if __name__ == '__main__':
    analysis_nlp_embedding()
